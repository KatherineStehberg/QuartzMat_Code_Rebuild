...
export function getServerCatalog() {
  return Object.fromEntries(
    Object.entries(PRODUCT_ENV_KEYS).map(([id, envKey]) => [id, { id, envKey, price: readPrice(envKey) }])
  );
}
...
export function calculateOrder(items) {
  ...
  for (const item of items) {
    ...
    if (product.price == null) throw new Error(`Falta configurar el precio de ${product.id}.`);
